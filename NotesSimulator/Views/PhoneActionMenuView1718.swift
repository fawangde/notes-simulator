...
struct PhoneActionMenuView1718: View {
        ...
        GeometryReader { geo in
            ZStack(alignment: .topLeading) {
                if presentation == .longPress {
                    ...
                    Color.black
                        .opacity(overlayOpacity * tokens.tapOverlayDim)
                        .ignoresSafeArea()
                ...
                if presentation == .tap, resolvedAnchor != .zero {
                    tapFog
                        .frame(width: resolvedAnchor.width + 8, height: resolvedAnchor.height + 4)
                        .offset(x: resolvedAnchor.minX - 4, y: resolvedAnchor.minY - 2)
                        .opacity(fogOpacity)
                }

                if presentation == .longPress, resolvedAnchor != .zero {
                    scaledShellCluster
                    morphTextLayer
                }
    ...
    private var morphTextLayer: some View {
        Text(formattedDialString)
            ...
            .lineLimit(1)
            .frame(width: resolvedAnchor.width, height: resolvedAnchor.height, alignment: .leading)
    ...
    private var tapFog: some View {
        RoundedRectangle(cornerRadius: tokens.tapFogCornerRadius, style: .continuous)
            .fill(tokens.tapFogColor)
            .allowsHitTesting(false)
    }
    ...
    private func runTapShow() {
        let final = PhoneMenu1718Layout.tapPlacement(anchor: resolvedAnchor, metrics: metrics)
        let spring = previewEnterSpring
    ...
    private func syncDismissBackdropOnly() {
        guard presentation == .longPress, isDismissing else { return }
