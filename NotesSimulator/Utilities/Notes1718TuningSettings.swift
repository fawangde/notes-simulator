...
/// iOS 17–18 备忘录 / 长按菜单调参（开发对照 reference 用）
struct Notes1718TuningSettings: Codable, Equatable {
    ...
    /// iOS 16–18 气泡尾巴样式（见 `IMessage1718BubbleTailPreset`）
    var bubbleTailPresetID: String = IMessage1718BubbleTailPreset.defaultPresetID
    ...
    /// 探针参考图叠加强度（BubbleRefTailCrop1718；撰写页点「探针」后生效）
    ...
    static let `default` = Notes1718TuningSettings()
    ...
    func paperBackgroundUIColor() -> UIColor {
        guard notesPaperDim > 0 || notesPaperWarmth > 0 else {
            return NotesStyle1718Tokens.paperUIColor
        }
        let gray6 = NotesStyle1718Tokens.paperUIColor
...
enum Notes1718TuningStore {
    private static let key = "Notes1718Tuning.v35"
    private static let legacyKey = "Notes1718Tuning.v34"
    private static let tailAnchorRevisionCurrent = 13

    static func load() -> Notes1718TuningSettings {
        let data = UserDefaults.standard.data(forKey: key)
            ?? UserDefaults.standard.data(forKey: legacyKey)
        guard let data,
              var settings = try? JSONDecoder().decode(Notes1718TuningSettings.self, from: data) else {
            return .default
        }
    ...
    /// 强制写回当前定稿描点（旧存档 / 手调残留）
    private static func migrateTailAnchorRevision(_ settings: Notes1718TuningSettings) -> Notes1718TuningSettings {
        var migrated = settings
        guard migrated.tailAnchorRevision < tailAnchorRevisionCurrent else { return migrated }
        migrated.resetTailAnchorDefaults()
        migrated.bubbleTailPresetID = IMessage1718BubbleTailPreset.defaultPresetID
    ...
    private static func migrateTailAnchorReferenceBody(_ settings: Notes1718TuningSettings) -> Notes1718TuningSettings {
        var migrated = settings
    ...
    private static func migrateTailAnchorTopology(_ settings: Notes1718TuningSettings) -> Notes1718TuningSettings {
        var migrated = settings
    ...
    private static func migrateBubbleTailTutorialParams(_ settings: Notes1718TuningSettings) -> Notes1718TuningSettings {
        var migrated = settings
    ...
    /// 旧预设 → 截图描边（唯一默认）
    private static func migrateBubbleTailPreset(_ settings: Notes1718TuningSettings) -> Notes1718TuningSettings {
        var migrated = settings
    ...
    static func save(_ settings: Notes1718TuningSettings) {
