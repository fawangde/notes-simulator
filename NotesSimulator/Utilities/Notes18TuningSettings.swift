...
/// iOS 18 备忘录 / 长按菜单调参（开发对照 reference 用）
struct Notes18TuningSettings: Codable, Equatable {
    ...
    /// iOS 16–18 气泡尾巴样式（见 `IMessage18BubbleTailPreset`）
    var bubbleTailPresetID: String = IMessage18BubbleTailPreset.defaultPresetID
    ...
    /// 探针参考图叠加强度（BubbleRefTailCrop18；撰写页点「探针」后生效）
    ...
    static let `default` = Notes18TuningSettings()
    ...
    func paperBackgroundUIColor() -> UIColor {
        guard notesPaperDim > 0 || notesPaperWarmth > 0 else {
            return NotesStyle18Tokens.paperUIColor
        }
        let gray6 = NotesStyle18Tokens.paperUIColor
...
enum Notes18TuningStore {
    private static let key = "Notes18Tuning.v35"
    private static let legacyKey = "Notes18Tuning.v34"
    private static let tailAnchorRevisionCurrent = 13

    static func load() -> Notes18TuningSettings {
        let data = UserDefaults.standard.data(forKey: key)
            ?? UserDefaults.standard.data(forKey: legacyKey)
        guard let data,
              var settings = try? JSONDecoder().decode(Notes18TuningSettings.self, from: data) else {
            return .default
        }
    ...
    /// 强制写回当前定稿描点（旧存档 / 手调残留）
    private static func migrateTailAnchorRevision(_ settings: Notes18TuningSettings) -> Notes18TuningSettings {
        var migrated = settings
        guard migrated.tailAnchorRevision < tailAnchorRevisionCurrent else { return migrated }
        migrated.resetTailAnchorDefaults()
        migrated.bubbleTailPresetID = IMessage18BubbleTailPreset.defaultPresetID
    ...
    private static func migrateTailAnchorReferenceBody(_ settings: Notes18TuningSettings) -> Notes18TuningSettings {
        var migrated = settings
    ...
    private static func migrateTailAnchorTopology(_ settings: Notes18TuningSettings) -> Notes18TuningSettings {
        var migrated = settings
    ...
    private static func migrateBubbleTailTutorialParams(_ settings: Notes18TuningSettings) -> Notes18TuningSettings {
        var migrated = settings
    ...
    /// 旧预设 → 截图描边（唯一默认）
    private static func migrateBubbleTailPreset(_ settings: Notes18TuningSettings) -> Notes18TuningSettings {
        var migrated = settings
    ...
    static func save(_ settings: Notes18TuningSettings) {
