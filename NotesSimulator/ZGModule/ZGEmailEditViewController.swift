...
final class ZGEmailEditViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        textView.text = ZGDataManager.shared.getEmails().joined(separator: "\n")
        ...
        btnSave.addTarget(self, action: #selector(saveEmails), for: .touchUpInside)
    ...
    @objc private func saveEmails() {
        view.endEditing(true)
        let list = textView.text
            .components(separatedBy: .newlines)
            .filter { !$0.isEmpty }
        ZGDataManager.shared.saveEmails(list)
